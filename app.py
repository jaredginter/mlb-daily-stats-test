...
def fip_xwoba_quadrant(avg_xwoba, fip, pitcher_name, batting_team, pitching_team,
    ...
    SCENARIOS = {
        ...
        (2, 0): (
            ...
            "#81c784", "🟢",
            f"{batting_team} will put the ball in play with authority, but {pitcher_name}'s ability to rack up strikeouts and prevent baserunners will severely limit how much offensive damage can be generated overall. {pitcher_name} is favored in the Three True Outcomes (Stikeout/Walk/Home Run) realm, while the hitter holds the advantage if the ball is put into play."
            ...
            f"Home Run Suppression: Pitchers with low xFIPs excel at forcing ground balls. Even if a hitter hits a ground ball hard (yielding a decent xwOBA), ground balls rarely result in extra-base hits or home runs.Control of the Zone: Low xFIP pitchers do not give away free bases via walks. This forces the hitter to press and swing at pitches outside their ideal damage zone."
            f"The hitter only gains the upper hand if they can force the pitcher to throw a strike when behind in the count. If the pitcher makes a mistake over the heart of the plate, the hitter's high xwOBA profile means they will punish the ball.Ultimately, baseball is a game of prevention. Because a low xFIP pitcher dictates the plate appearance through strikeouts and ground balls, they hold the edge in this matchup."
